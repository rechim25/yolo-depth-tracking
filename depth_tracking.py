...
def run_yolo_tracking_with_depth(args):
    ...
    # # Track
    # results = model.track(
    #     source="/home/rechim/object-tracking/240517_062019_062029.mp4",
    #     save=True,
    ...
    try:
        ...
        while raw_video.isOpened():
            ...
            for box, track_id in zip(boxes, track_ids):
                ...
                if x < 0 or x >= frame.shape[0] or y < 0 or y >= frame.shape[0]:
                ...
                print(f"depth image at x,y: {tuple(depth_image[x, y])}")
                cv2.polylines(annotated_frame, [points], isClosed=False, color=tuple(
                    int(c) for c in depth_image[x, y]), thickness=10)
...
#     output_path = os.path.join(args.outdir, os.path.splitext(
#         os.path.basename(filename))[0] + '.mp4')
#     out = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(
#         *"mp4v"), frame_rate, (output_width, frame_height))
