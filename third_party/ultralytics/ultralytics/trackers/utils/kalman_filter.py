...
class KalmanFilterXYZAH:
    """
    A Kalman filter for tracking bounding boxes in image space, extended to include depth (z).

    State vector (10D): [x, y, z, a, h, vx, vy, vz, va, vh]
    Measurement vector (5D): [x, y, z, a, h]
    """

    def __init__(self):
        """
        Initializes the Kalman filter matrices with depth included.
        """
        ndim, dt = 5, 1.0  # Now state vector has 5 position variables
        ...
        # Define the standard deviations for the process and measurement noise
    ...
    def initiate(self, measurement):
        ...
        Args:
            measurement (ndarray): [x, y, z, a, h]

        Returns:
            mean (ndarray): 10D state vector
            covariance (ndarray): 10x10 covariance matrix
        """
        ...
        std_pos = [
            ...
            1e-1,  # z (Assuming a fixed small std for depth)
            1e-2,  # a (aspect ratio)
        ...
        std_vel = [
            ...
            1e-2,  # vz (Assuming a fixed small std for depth velocity)
            1e-5,  # va
    ...
    def predict(self, mean, covariance):
        ...
        Returns:
            mean (ndarray): Predicted state vector
            covariance (ndarray): Predicted covariance matrix
        """
        std_pos = [
    ...
    def project(self, mean, covariance):
        ...
        Returns:
            mean (ndarray): Projected 5D measurement vector
            covariance (ndarray): Projected 5x5 covariance matrix
        """
        std = [
            self._std_weight_position * mean[4],  # x
            self._std_weight_position * mean[4],  # y
    ...
    def update(self, mean, covariance, measurement):
        ...
        Returns:
            mean (ndarray): Updated state vector
            covariance (ndarray): Updated covariance matrix
        """
    ...
    def gating_distance(self, mean, covariance, measurements, only_position=False, metric='maha'):
        ...
        Args:
            mean (ndarray): 10D state vector
            covariance (ndarray): 10x10 covariance matrix
            measurements (ndarray): Nx5 array of measurements
            only_position (bool): If True, only use x, y, z for gating
            metric (str): 'maha' for Mahalanobis distance

        Returns:
            distances (ndarray): Gating distances
        """
        projected_mean, projected_cov = self.project(mean, covariance)
    ...
    def multi_predict(self, means, covariances):
        """
        Vectorized prediction for multiple states.

        Args:
            means (ndarray): Nx10 array of state vectors
            covariances (ndarray): Nx10x10 array of covariance matrices

        Returns:
            means (ndarray): Predicted state vectors
            covariances (ndarray): Predicted covariance matrices
        """
        std_pos = [
            self._std_weight_position * means[:, 4],
            self._std_weight_position * means[:, 4],
            1e-1 * np.ones(len(means)),
            1e-2 * np.ones(len(means)),
            self._std_weight_position * means[:, 4],
        ]
        std_vel = [
            self._std_weight_velocity * means[:, 4],
            self._std_weight_velocity * means[:, 4],
            1e-2 * np.ones(len(means)),
            1e-5 * np.ones(len(means)),
            self._std_weight_velocity * means[:, 4],
        ]
        ...
        covariances = np.matmul(np.matmul(self._motion_mat, covariances), self._motion_mat.T) + motion_cov
